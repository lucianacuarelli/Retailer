...
class Module extends UseZF_Module_Abstract {
    ...
    public function login($params) 
    {
		...
		if($login->status == 'success')
		{
    		...
    		$this->useDB('UPDATE_LAST_LOGIN')->data(array('last_login' => date('Y-m-d H:i:s')))->update('register', 'id = '. $login->info->id);
			...
			if(!empty($login->info->parent_id))
			{
				...
				$useResultCorporate = $this->useDB('SELECT')
				                	       ...
				                	       ->fields(array('rede','fancy_name','corporate_name','cnpj','ie','ie_exemption','network'))
    ...
    public function showMessage($params) 
    {
    	...
    	$useResult = $this->useDB('SELECT')
						  ...
						  if(!empty($params->search)) $this->useDB('SELECT')->where("USE_REGISTER_MESSAGES.message LIKE '%". $params->search ."%'");
    ...
    public function showMessageList($params) 
    {
    	...
    	$useResult = $this->useDB('SELECT')
						  ...
						  ->order('USE_REGISTER_MESSAGES.created_in ASC')
            		      ->fetch('All');
		...
		if($useResult[0]->is_close == 1)
		{
			//Verifica se o fechamento da mensagem recebeu nota
			if($useResult[0]->close_note == 0)
			{
