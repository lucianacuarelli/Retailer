...
class Module extends UseZF_Module_Abstract {
    ...
    public function dbUpdate() 
    {
    	...
    	//----------------------------------------------------------------------------
			...
			foreach ($useResultPersonal as $item) {
				//$item->url_active_created_in = strtotime($item->url_active_created_in);
				//$item->url_referer_created_in = strtotime($item->url_referer_created_in);
				//$item->session_created_in = strtotime($item->session_created_in);
				$register_id = $this->useDB('SAVE_REGISTER','parceiro')->data($item)->insert('lg_analytcs');
			}*/
		//----------------------------------------------------------------------------
		// lg_analytcs_email_friend
    	//----------------------------------------------------------------------------
			/*$useResultPersonal = $this->useDB('SELECT')
	                	   			  ->setFetchMode(Zend_Db::FETCH_OBJ)
			                	      ->from(array('USE_LG_ANALYTCS_EMAIL_FRIENDS' => 'lg_analytcs_email_friend'))
			                	      ->fields(array('*'))
									  ->fetch('All');
			...
			foreach ($useResultPersonal as $item) {
				...
				//$item->url_referer_created_in = strtotime($item->url_referer_created_in);
				//$item->session_created_in = strtotime($item->session_created_in);
				$register_id = $this->useDB('SAVE_REGISTER','parceiro')->data($item)->insert('lg_analytcs_email_friend');
			}*/
    ...
    public function showComment($params) 
    {
    	$useResult = $this->useDB('SELECT')
					      ...
					      ->where('USE_LG_COMMENTS.lg_resources_id = ?', $params->lg_resources_id)
    ...
    public function showMessage($params) 
    {
		...
		*/
		...
		foreach ($useResult as $key => $value) 
		{
			...
			if($useRegisterMessagesTotal->total == 0)
			{
				$register_to_name = $area = $status = "Aguardando interação"; 
			}
			else{
				$register_to_name = $useRegisterMessagesFirst->name;
				$area = $useRegisterMessagesFirst->title;
				
				if($useRegisterMessagesFirst->is_close == 1)
				{
					if($useRegisterMessagesFirst->close_note == 0)
					{
			...
			$hourOpen = explode(' ', $value->created_in);
			$dateOpen = explode('-', $hourOpen[0]);
			$dateOpen = $dateOpen[2] .'/'. $dateOpen[1] .'/'. $dateOpen[0] .' '. $hourOpen[1];
			
			//Configura a data da ultima interacao
			if(!empty($useRegisterMessagesLast))
			{
				$hourOpenLast = explode(' ', $useRegisterMessagesLast->created_in);
				$dateOpenLast = explode('-', $hourOpenLast[0]);
				$dateOpenLast = $dateOpenLast[2] .'/'. $dateOpenLast[1] .'/'. $dateOpenLast[0] .' '. $hourOpenLast[1];
			}
			else {
			...
			$html .= '<tr'. $class .'>
					  ...
					  <tr'. $class .'>
					    <td><strong>Assunto:</strong> '. $value->subject .'</td>
    ...
    public function showMessageList($params) 
    {
		...
		$html = '<div class="UiMessage">
					<div class="usePageTitle">
						<h3>'. $useResult[0]->subject .'</h3>
					</div>
					<div class="usePageContent">';
				
		//Verifica se a mensagem esta fechada
		if($useResult[count($useResult)-1]->is_close == 1)
		{
			//Verifica se o fechamento da mensagem recebeu nota
			if($useResult[count($useResult)-1]->close_note <= 0)
			{
		...
		foreach ($useResult as $key => $value) 
		{
			...
			$dateFull = UseZF_Util::useDateElapsed(strtotime($value->created_in), strtotime(date('Y-m-d H:i:s')));
